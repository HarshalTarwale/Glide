...
import { createBillFromOrderAction, type ActionResult } from "../../actions";
...
export function CreateBillDialog({
  ...
  orderId: string;
  /** Only lines with something left to bill, per the order's own billingPolicy. */
  lines: BillableOrderLineDTO[];
}) {
  const action = createBillFromOrderAction.bind(null, orderId);
  ...
  const payload = lines.map((l) => ({ purchaseOrderLineId: l.id, quantity: quantities[l.id] ?? 0 })).filter((l) => l.quantity > 0);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Adjust the quantity per line, or leave the full remaining amount to bill everything outstanding.</DialogDescription>
        </DialogHeader>
        ...
        <form action={formAction}>
          ...
          <DialogBody className="space-y-3">
            {lines.length === 0 ? (
              <p className="text-sm text-ink-muted">Nothing on this order is billable yet.</p>
            ) : (
              lines.map((line) => (
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <Input id="billDueDate" name="dueDate" type="date" />
            </div>
