...
import type { BillPaymentDTO, OpenBillDTO } from "@/server/procurement/bill-payments";
...
export function PaymentView({ payment, openBills }: { payment: BillPaymentDTO; openBills: OpenBillDTO[] }) {
    ...
    <RecordShell
      header={
        <PageHeader
          ...
          crumbs={[{ label: "Procurement", href: "/app/procurement/orders" }, { label: "Payments", href: "/app/procurement/payments" }, { label: payment.number }]}
          status={
          ...
          meta={
            <span className="flex flex-wrap items-center gap-x-4 gap-y-1">
              ...
              <span>
                Paid <DateText value={payment.paymentDate} />
              </span>
            </span>
          }
          actions={
            ...
            {payment.allocations.length === 0 ? (
              <tr>
                <td colSpan={2} className="px-3 py-6 text-center text-sm text-ink-subtle">
                  This payment has not been applied to any bill yet.
                </td>
              </tr>
            ) : (
              payment.allocations.map((a) => (
                <tr key={a.id} className="h-row border-b border-hairline last:border-0">
                  <td className="px-3">
                    <Link href={`/app/procurement/bills/${a.billId}`} className="text-accent hover:underline">
                      <Code>{a.billNumber}</Code>
                    </Link>
                  </td>
