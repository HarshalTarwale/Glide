...
import type { ApAgingReportDTO } from "@/server/procurement/ap-aging";

const BUCKETS: { key: keyof ApAgingReportDTO["totals"]; label: string }[] = [
...
export function AgingView({ report }: { report: ApAgingReportDTO }) {
  if (report.partners.length === 0) {
    return (
      <>
        <PageHeader title="AP Aging" crumbs={[{ label: "Procurement", href: "/app/procurement/bills" }, { label: "AP Aging" }]} />
        <EmptyState icon={CircleDollarSign} title="Nothing outstanding" description="No posted bill has a balance due right now." />
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader
        title="AP Aging"
        crumbs={[{ label: "Procurement", href: "/app/procurement/bills" }, { label: "AP Aging" }]}
        meta={
          <span>
                ...
                {report.partners.map((p) => (
                  <tr key={p.partnerId} className="h-row border-b border-hairline last:border-0">
                    <td className="px-3 font-medium text-ink">
                      <Link href={`/app/procurement/bills?search=${encodeURIComponent(p.partnerName)}`} className="hover:underline">
                        {p.partnerName}
                      </Link>
                    </td>
                    <td className="tnum px-3 text-right text-ink-muted">{p.billCount}</td>
                    {BUCKETS.map((b) => (
