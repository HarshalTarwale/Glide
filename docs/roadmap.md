...
- **Background jobs (pg-boss)** — no actual job exists yet in the product (no webhooks, nothing scheduled). Building the queue now would be speculative infrastructure with nothing to run; better to add it once Stripe webhooks give it one. (Invitation email sending, once the one real candidate for this queue, turned out not to need it — a single `fetch` at invite time is fast enough that a queue would be premature.)
...
- **Auto-posting**: `bill.posted` → Dr Inventory Asset (+ Tax Payable, netted down rather than booked to a separate Input Tax Credit asset — a stated v1 simplification) / Cr Accounts Payable — updated 2026-09-05 from Cost of Goods Sold to Inventory Asset, see the stock-move-events fix below. `billpayment.recorded` → Dr Accounts Payable / Cr Cash. Idempotent via the same `(tenantId, sourceType, sourceId)` unique constraint the Accounting section already established.
...
#### Stock-move domain events — **built, confirmed with the user 2026-09-05**

Closes the gap Accounting/GL, Procurement, and Manufacturing's own scope notes had all separately named as future work: "stock movements don't yet emit domain events, so COGS/inventory postings aren't automatic." Not a new module -- a fix threaded through four existing ones.

- **`recordMove()` (src/server/inventory/stock.ts) now returns `{ move, valuationEvent }`** instead of a bare move -- `valuationEvent` is a new `stock.valued` domain event (src/server/core/events.ts), populated whenever a move crosses the company's ownership boundary (exactly the moves that already get a `StockValuationLayer` -- an internal transfer still gets nothing). Emitted by the CALLER after its own transaction commits, never from inside `recordMove` itself -- it runs inside a caller-owned transaction that might still roll back, so `receiveStock`/`deliverStock`/`transferStock`/`adjustStock` and the three cross-module callers (`sales/orders.ts`'s `createDelivery`, `procurement/orders.ts`'s `createReceipt`, `manufacturing/work-orders.ts`'s `completeWorkOrder`) all collect the event(s) and `emit()` only after `withTenant()` resolves, matching the exact convention `invoicing.ts`'s `postInvoice` already established.
- **`gl-subscriber.ts` gained one handler, `stock.valued`, that posts something only for `moveType: "delivery"`**: Dr Cost of Goods Sold / Cr Inventory Asset, at the AVCO cost the move was actually valued at -- the first time ANYTHING has recognised a cost of sale in this codebase; `invoice.posted` only ever recognised revenue. Every other `moveType` is deliberately ignored, stated in the handler's own comment rather than left to be reverse-engineered: `"receipt"` is already captured by `bill.posted` (below); `"adjustment"` is real, unbuilt v2 scope (a shrinkage/count-correction account); `"consumption"`/`"production"` (Manufacturing) net to exactly zero within Inventory Asset by construction, since a work order's produced unit cost IS `totalComponentCost / quantity` (`src/lib/manufacturing/bom.ts`) -- posting a journal entry for that would be correct but pointless, a debit and credit of the same amount on the same account.
- **`bill.posted` changed from Dr Cost of Goods Sold to Dr Inventory Asset**, Cr Accounts Payable unchanged. A purchase increases what the company owns; it was never an expense until this fix made COGS actually mean "cost of what was SOLD" rather than "cost of what was BOUGHT." The two together now match the textbook perpetual-inventory shape: buy → asset up; sell → asset down, expense up.
- **One stated, honest remaining gap, not silently left out**: Inventory Asset is recognised at BILL time, not physical receipt time -- goods received but not yet billed sit uncosted in the GL for however long that gap is. A true three-way PO/Receipt/Bill match with GR/IR clearing is real v2 scope; building it now would have turned a bounded fix into a new feature.

Verified against live Neon: `tests/stock-gl.test.ts` (3/3 -- a receipt posts nothing by itself while a delivery posts the exact Dr COGS/Cr Inventory Asset entry at AVCO cost; a bill and a later delivery net correctly in the SAME Inventory Asset account on a real Trial Balance, proving the whole redesign coheres end to end; completing a manufacturing work order posts no journal entry at all, confirmed rather than assumed), plus `tests/procurement.test.ts`'s existing bill-posting test updated to assert Inventory Asset instead of Cost of Goods Sold. Full suite 277/280 (3 skipped, unchanged), tsc clean, eslint clean, `next build` clean. Checked against both real (non-test) tenants directly in Neon: neither has ever posted a bill, so this change requires no backfill for existing data.
